...
import { setResults } from "utilities/globalState";
...
describe("search bar", () => {
    test("correctly processes search tags and ranges", async () => {
        render(<Suspense
            fallback="Suspense"><HomePage /></Suspense>);
        
        const searchField_DOM = screen.getByTestId(/search-field/i);
        const searchButton_DOM = screen.getByTestId(/search-button/i);
        
        // Wait for tags to load
        await waitFor(() => expect(screen.queryAllByTestId(/tag-category-list/i)[0]).toBeInTheDocument());
    ...
    test("correctly renders used tags", async () => {
        ...
        await waitFor(() => {
            const usedTagItemAll_DOM = screen.queryAllByTestId(/used-tag-item/i);
