...
import { setIsSignedIn } from "utilities/globalState";

/// Tests
// Controls
describe("controls", () => {
    test("render correctly", async () => {
        render(<div data-testid="test-wrapper"><Controls /></div>);

        // Check that edit button does not exist when not signed in
        expect(screen.queryByTestId("controls-edit-btn")).not.toBeInTheDocument();
        ...
        expect(screen.queryByTestId("controls-cancel-icon")).not.toBeInTheDocument();
        expect(screen.queryByTestId("controls-save-btn")).not.toBeInTheDocument();
        ...
        expect(screen.queryByTestId("controls-cancel-icon")).toBeInTheDocument();
    });
});
